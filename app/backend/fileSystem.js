...
export const create_book = async (bookName, libName, imageUri) => {
    const filesData = await DocumentPicker.getDocumentAsync({multiple: true, type: "application/*"});
    ...
    if (filesData.canceled != true) {
        ...
        if (files.length > 1) {
            for (let x = 0; x < files.length; x++) {
                const uri = files[x];
                const destinationUri = libName == "All" ? `${allPath}/${bookName}` : `${allPath}/${libName}/${bookName}`;
...
export const pickImage = async () => {
    const getImageAfterPerms = async (perms) => {
        if (perms.accessPrivileges == "all" || perms.accessPrivileges == "limited") {
            const imageData = await ImagePicker.launchImageLibraryAsync({allowsEditing: true});
...
export const delete_book = async (lib, bookName) => {
    const deletePath = lib == "All" ? `${FileSystem.documentDirectory}All/${bookName}` : `${FileSystem.documentDirectory}All/${lib}/${bookName}`;
